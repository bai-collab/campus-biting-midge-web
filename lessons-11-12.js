...
function teacher(){show("teacherView");renderTeacher();}
function allStored(){const a=[];for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i);if(k?.startsWith("bmt_")){try{a.push(JSON.parse(localStorage.getItem(k)))}catch{}}}return a.sort((x,y)=>(x.classId+x.groupId).localeCompare(y.classId+y.groupId))}
function renderTeacher(){const arr=allStored();groupList.innerHTML=arr.length?"":"<div class='note'>目前沒有任何單機測試資料。</div>";arr.forEach(d=>{const done=Object.values(d.lessons).filter(x=>x.completed).length;const row=document.createElement("button");row.className="group-row";row.style="width:100%;border-left:0;border-right:0;border-top:0;background:white;text-align:left";row.innerHTML=`<span>${clsNames[d.classId]} ${groupNames[d.groupId]}</span><span>${done}/12</span><span>第 ${d.lastLesson||1} 節</span><span>${d.updatedAt?new Date(d.updatedAt).toLocaleTimeString("zh-TW",{hour:"2-digit",minute:"2-digit"}):"—"}</span>`;row.addEventListener("click",()=>preview(d,row));groupList.appendChild(row)})}
function preview(d,row){document.querySelectorAll("#groupList .group-row").forEach(x=>x.classList.remove("active"));row.classList.add("active");const f1=d.lessons["1"].fields||{},f2=d.lessons["2"].fields||{};const done=Object.values(d.lessons).filter(x=>x.completed).length;previewMount.innerHTML=`<h3>${clsNames[d.classId]} ${groupNames[d.groupId]}｜學習單預覽</h3><p class="mini">已完成 ${done}/12 節｜最後更新 ${d.updatedAt?new Date(d.updatedAt).toLocaleString("zh-TW"):"—"}</p><div class="preview-section"><h4>第 1 節｜我的小黑蚊經驗 ${d.lessons["1"].completed?"✓":""}</h4><div class="body">時間：${f1.event_time||"—"}\n地點：${f1.event_place||"—"}\n事件：${f1.integrated_event||f1.event_detail||"—"}\n影響：${f1.impact||"—"}</div></div><div class="preview-section"><h4>第 2 節｜校園叮咬熱區踏查 ${d.lessons["2"].completed?"✓":""}</h4><div class="body">觀察點：${f2.point_id||"—"}　${f2.location||""}\n觀察現象：${f2.phenomenon||f2.biting_detail||"—"}\n反思：${f2.can_say||"—"}</div></div>`}
enterBtn.addEventListener("click",enter);
homeBtn.addEventListener("click",()=>{if(localStorage.getItem(keyOf())){renderDash();show("dashView")}else show("loginView")});
teacherBtn.addEventListener("click",teacher);refreshTeacher.addEventListener("click",renderTeacher);
